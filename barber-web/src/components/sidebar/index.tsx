...
export function Sidebar({ children }: { children: ReactNode }) {
    ...
    return (
        <Box minH="100vh" background="barber.900">
            <SidebarContent
                onClose={() => onClose}
            ...
            <Box ml={{ base: 0, md: 60 }} p={4}>
                {/* children é o local das pagina separadas do sidebar */}
                {children}
            </Box>
        </Box>
    )
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => { // ..rest = todo restante das propiedades do BoxProps
    return (
        <Box
            color="white"
...
const MobileNav = ({ onOpen, ...rest }: MobileProps) => {
    return (
        <Flex
            ...
            height="20"
            alignItems="center"
            bg={useColorModeValue('barber.200', 'barber.700')}
            borderBottomWidth="1px"
            borderBottomColor={useColorModeValue('gray.800', 'gray.600')}
